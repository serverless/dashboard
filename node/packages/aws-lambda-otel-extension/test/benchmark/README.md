...
- Comparing durations of `console` vs `protoLog` expose the overhead of the communication with ingestion server that happens in external extension
...
- `SLS_ORG_NAME` & `SLS_ORG_TOKEN`- (optional) Needed to benchmark scenario of reporting to the Console Kinesis server (if not provided, `console` variant will not be tested)
