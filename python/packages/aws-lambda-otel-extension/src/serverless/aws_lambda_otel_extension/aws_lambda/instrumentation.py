                            ...
                            "spans": [
                                {
                                    ...
                                    "name": "aws-node-http-api-project-srs-dev-hello",
                                }
                            ],
                        }

In Python (instrumented span)
                        {
                            "spans": [
                                {
                                    "name": "wsgi_handler.handler",
                                }
                            ],
                        },
...
def _instrument(
    ...
    def _instrumented_lambda_handler_call(call_wrapped, instance, args, kwargs):
        ...
        # Add in a sprinkle of extra accuracy.
        start_time = _time_ns()
        ...
        try:
            with temporary_tracer.start_as_current_span(name="detect") as detect_span:
        ...
        try:
            with temporary_tracer.start_as_current_span(name="extract") as extract_span:
        ...
        if store.is_cold_start:
            ...
            for pre_instrumentation_span in pre_instrumentation_spans:
                # Rewind the start time a smidge if this was a cold start and we have some spans that have already been
                # created.
                if pre_instrumentation_span.start_time < start_time:
        ...
        with package_tracer.start_as_current_span(
            ...
            if instrumentation_span.is_recording():
                ...
                if execution_id:
                    instrumentation_span.set_attribute(SpanAttributes.FAAS_EXECUTION, execution_id)
            ...
            base_attributes = BoundedAttributes(
                attributes=_filtered_attributes(
                    attributes={
                        ...
                        "eventCustomRequestId": execution_id,
            ...
            with tracer.start_as_current_span(
                name="start",
                attributes={
                    SlsExtensionSpanAttributes.SLS_SPAN_TYPE: "start",
                },
            ) as start_span:
            ...
            # Flush to make sure the event data is transmitted.
            try:
                _force_flush()
            except Exception:
            ...
            with tracer.start_as_current_span(
                name="finish",
                attributes={
                    SlsExtensionSpanAttributes.SLS_SPAN_TYPE: "finish",
                },
            ) as finish_span:
